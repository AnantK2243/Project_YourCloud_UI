...
export class NodeService {
	...
	async registerNode(node_name: string): Promise<{
		...
		if (
			!node_name ||
			this.userStorageNodes.value.find((d) => d.node_name === node_name)
		) {
			return {
				success: false,
		...
		try {
			...
			if (response && response.success) {
				...
				return {
					success: false,
					message: response?.error || "Node registration failed",
				};
		} catch (error: any) {
			return {
				success: false,
				message: error?.message || "Node registration failed",
			};
		}
	}
	...
	async loadUserStorageNodes(): Promise<{
		...
		try {
			...
			if (response && response.success) {
				...
				return {
					success: false,
					message: response?.error || "Failed to fetch storage nodes",
				};
			}
		} catch (error: any) {
			this.userStorageNodes.next([]);
	...
	async deleteStorageNode(
		...
		try {
			...
			if (response && response.success) {
				this.userStorageNodes.next(
					this.userStorageNodes.value.filter(
						(n) => n.node_id !== nodeId
					)
				);
				return { success: true };
				...
				return {
					success: false,
					message: response?.error || "Failed to delete storage node",
				};
			}
		} catch (error: any) {
			return {

...
export class SessionStorageService {
  ...
  private isSessionValid(data: SessionData): boolean {
    ...
    return sessionAge <= this.SESSION_TIMEOUT && !!data.sessionId && !!data.encryptedPassword && !!data.salt;
  }
  ...
  async retrieveCredentials(): Promise<{password: string; salt: string;} | null> {
    if (!this.isBrowser()) return null;
